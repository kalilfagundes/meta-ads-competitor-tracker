...
Each container gets only the secrets it needs: Postgres only its password, Garage only its RPC
secret and admin token, and the worker no session secret.

The app's containers get `POSTGRES_PASSWORD`, `S3_SECRET_ACCESS_KEY`, `SESSION_SECRET` and
`GARAGE_ADMIN_TOKEN` as [Compose secrets](https://docs.docker.com/compose/how-tos/use-secrets/)
sourced from `.env`: read-only files under `/run/secrets`, not in their environment. The app reads
them through `*_FILE` variables (`POSTGRES_PASSWORD_FILE=/run/secrets/postgres_password`, …).

Postgres and Garage get theirs as plain variables. Their images refuse a secret given both ways,
and hosts that also hand all of `.env` to every container, like Coolify, would do exactly that.
The app takes either and prefers the variable.
...
Actor on your Apify account, which reads through residential proxies. The help page linked next to
the token field names the Actor too, so you always know what runs on your account.
...
- **Restarts.** Actor runs go on on Apify when the worker stops (a restart, a deploy, a crash),
  so the collection is left paused and the next worker picks it up: it waits for the Actor runs
  still going, stores the ads of those that finished and starts only the Batches never sent.
  Nothing is paid for twice. A collection left for more than a day is closed instead, and the
  next one collects afresh.
...
The app serves plain HTTP on port `8765` of your machine (`WEB_PORT`); a proxy running in Docker,
like Coolify's, reaches the `web` container on port `8000` instead. Before exposing it beyond your
machine:
...
machine or in Docker, while a visitor reaching port 8765 straight from the internet can't fake
...
uvicorn tracker.web.app:app --host 0.0.0.0 --port 8765    # web
