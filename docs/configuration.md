...
[Facebook (Meta) Ads Library Scraper](https://apify.com/kalilfagundes.w/facebook-instagram-meta-ads-library-scraper)
Actor on your Apify account, which reads through residential proxies.
...
- **Batches.** Pages are sent to the Actor 5 at a time (one country per run). *Simultaneous Actor
  runs* (1–5, default 2) sets how many of those runs go at once. Apify's free plan allows at most 5
