...
- **`test_db.py`** — the data layer (the ads library: filters, pages, Versions, champions; ad detail with formats and
  ...
  single-flight "collect now", the worker lock, interrupted and resumed runs, collections)
  plus the landing-page title crawl against the database (network stubbed).
  Needs a Postgres.
