...
def test_filters_take_several_companies(client, seeded):
    lp_link = f'href="/landing-pages/{seeded["lp"]}"'
    northwind, contoso = seeded["companies"]["northwind"], seeded["companies"]["contoso"]
    assert lp_link not in client.get(f"/landing-pages?company={contoso}").text
    assert lp_link in client.get(f"/landing-pages?company={contoso}&company={northwind}").text

    both = client.get("/api/ads", params=[("status", "all"), ("company", "Contoso Academy"),
                                          ("company", "Northwind Prep")]).json()
    one = client.get("/api/ads", params={"status": "all", "company": "Contoso Academy"}).json()
    assert (both["total"], [c["id"] for c in one["cards"]]) == (4, ["AD_CAROUSEL"])
    # the page comes with those companies ticked
    page = client.get("/", params=[("company", "Contoso Academy"), ("company", "Northwind Prep")]).text
    assert page.count(" checked>") == 2


def test_adding_a_company_with_its_first_page(client, db):
    r = client.post("/admin/companies", data={"name": "Acme", "domain": "", "page_id": "12345",
                                              "collect_country": "mx"}, follow_redirects=False)
    assert r.status_code == 302 and "err=" not in r.headers["location"]
    page = db.execute("""SELECT p.page_id, p.collect_country, c.name
                         FROM pages p JOIN companies c ON c.id = p.company_id""").fetchone()
    assert (page["page_id"], page["collect_country"], page["name"]) == ("12345", "MX", "Acme")
    # a page ID that isn't one is refused, and nothing is created
    r = client.post("/admin/companies", data={"name": "Beta", "page_id": "facebook.com/beta"},
                    follow_redirects=False)
    assert r.headers["location"] == "/admin?err=page_id_digits"
    # the page ID is optional
    assert "err=" not in client.post("/admin/companies", data={"name": "Gamma"},
                                     follow_redirects=False).headers["location"]
    assert [c["name"] for c in db.execute("SELECT name FROM companies ORDER BY id").fetchall()] == ["Acme", "Gamma"]
    gamma = db.execute("SELECT id FROM companies WHERE name = 'Gamma'").fetchone()["id"]
    r = client.post(f"/admin/companies/{gamma}/pages", data={"page_id": "abc"}, follow_redirects=False)
    assert r.headers["location"] == "/admin?err=page_id_digits"
...
def test_collect_now_is_single_flight(client, seeded):
    ...
    # the plain form post (no JS) redirects back to Settings, saying why nothing started
    r = client.post("/admin/collect-now", follow_redirects=False)
    assert r.status_code == 302 and r.headers["location"] == "/admin?err=collect_busy"


def test_collect_now_refused_by_a_run_in_flight_names_it(client, seeded, db):
    from tracker.worker import persist

    run_id = persist.start_collection_run(db, trigger="schedule", pages_total=1)
    body = client.post("/admin/collect-now", headers={"Accept": "application/json"}).json()
    assert (body["accepted"], body["state"], body["run"]["id"]) == (False, "running", run_id)
