...
def get_run_status(limit: int = 10) -> dict:
    ...
    run stopped sending heartbeats) | 'idle'. Also carries the live progress of the
    run in flight (else the latest run) and the recent runs.
    """
    with connect() as conn:
        s = conn.execute(
    ...
    # The run on show is the one in flight, even behind a newer run that finished:
    # while it's open, "collect now" is refused (`request_collect_now`), and the
    # screen must say why.
    in_flight = next((r for r in runs if r["finished_at"] is None and not r["crashed"]), None)
    current = in_flight or (runs[0] if runs else None)
    ...
    request_pending = (
        ...
        and (current is None or requested_at > current["started_at"])
    )
    state = _run_state(bool(s["collect_now"]), request_pending, current)

    next_run_in_s = None
    if state == "idle" and current is not None and current["since_finish_s"] is not None:
        interval_s = max(1, int(s["collect_interval_hours"] or 6)) * 3600
        next_run_in_s = max(0, interval_s - current["since_finish_s"])
