...
(function () {
  ...
  var DATA = window.__LIBRARY__ || { cards: [], champions: [], companies: [], total: 0, page: 0, pages: 1 };
  ...
  // company dropdown (alphabetical), pre-selected from ?company= if present
  (function () {
    var sel = $("f-company");
    ...
    if (DATA.company && (DATA.companies || []).indexOf(DATA.company) !== -1) sel.value = DATA.company;
  ...
  function render() {
    var co = $("f-company").value;
    rankHint();
    $("champ-note").textContent = co ? _("Top ads from %(company)s", { company: co }) : _("Each competitor's longest-running ads");
