...
# Why a run was closed without finishing, and the steps of a run through Apify
# that a stopped worker left for the next one. The Settings screen translates them
# (static/js/admin.js), so keep the two in step.
RUN_INTERRUPTED = "Interrupted: the worker stopped before this run finished."
RUN_STOPPED = "Interrupted: the worker was stopped (restart or deploy) before this run finished."
STEP_PAUSED = "Paused until the worker is back"
STEP_RESUMING = "Resuming the Apify runs"


def close_orphan_runs(conn: psycopg.Connection, stale_minutes: int | None = db.STALE_MINUTES,
                      reason: str = RUN_INTERRUPTED, keep=()) -> int:
    """Close unfinished runs as interrupted, so they don't show as running forever.

    With `stale_minutes`, only runs with no heartbeat for that long (a worker
    crashed mid-run); a run still beating is left alone. None closes every
    unfinished run: only for a worker holding the worker lock, which knows any open
    run is its own or a dead worker's. Runs in `keep` (ids) are left open. Returns
    how many were closed."""
    rows = conn.execute(
        """UPDATE collection_runs
           SET finished_at = COALESCE(heartbeat_at, started_at), errors = errors + 1,
               last_error = %(reason)s, current_step = NULL, current_page_ads = 0
           WHERE finished_at IS NULL
             AND NOT (id = ANY(%(keep)s::int[]))
             AND (%(stale)s::int IS NULL
                  OR COALESCE(heartbeat_at, started_at) < now() - make_interval(mins => %(stale)s::int))
           RETURNING id""",
        {"reason": reason, "stale": stale_minutes, "keep": [k for k in keep if k is not None]},
    ).fetchall()
    conn.commit()
    return len(rows)


# ── Apify Actor runs of a collection run (so another worker can resume it) ───

def record_actor_run(conn: psycopg.Connection, run_id: int, actor_run: dict, country: str,
                     page_ids: list[int], attempt: int) -> None:
    """An Actor run a collection run started for one Batch (its country and Pages)."""
    conn.execute(
        """INSERT INTO apify_actor_runs (id, collection_run_id, country, page_ids, attempt,
                                         dataset_id, key_value_store_id)
           VALUES (%s, %s, %s, %s, %s, %s, %s) ON CONFLICT (id) DO NOTHING""",
        (actor_run["id"], run_id, country, page_ids, attempt,
         actor_run.get("defaultDatasetId"), actor_run.get("defaultKeyValueStoreId")),
    )
    conn.commit()


def conclude_actor_run(conn: psycopg.Connection, actor_run_id: str, outcome: str) -> None:
    """Its Batch is dealt with: 'stored' (its ads are saved) or 'failed' (the Batch
    gave up and its error is counted)."""
    conn.execute("UPDATE apify_actor_runs SET outcome = %s WHERE id = %s", (outcome, actor_run_id))
    conn.commit()


def actor_runs_of(conn: psycopg.Connection, run_id: int) -> list[dict]:
    """The Actor runs a collection run started, oldest first."""
    return conn.execute(
        """SELECT id, country, page_ids, attempt, outcome FROM apify_actor_runs
           WHERE collection_run_id = %s ORDER BY started_at, attempt""",
        (run_id,),
    ).fetchall()


def resumable_run(conn: psycopg.Connection, within_hours: int) -> int | None:
    """The latest unfinished collection run that started Actor runs, if it started
    within the last `within_hours`."""
    row = conn.execute(
        """SELECT r.id FROM collection_runs r
           WHERE r.finished_at IS NULL
             AND r.started_at > now() - make_interval(hours => %s)
             AND EXISTS (SELECT 1 FROM apify_actor_runs a WHERE a.collection_run_id = r.id)
...
def pause_apify_runs(conn: psycopg.Connection) -> list[int]:
    """Leave the unfinished runs that started Actor runs open, saying they wait for
    a worker to resume them (their Actor runs go on on Apify). Returns their ids."""
    rows = conn.execute(
        """UPDATE collection_runs r
           SET current_step = %s, current_page_ads = 0, heartbeat_at = now()
           WHERE r.finished_at IS NULL
             AND EXISTS (SELECT 1 FROM apify_actor_runs a WHERE a.collection_run_id = r.id)
           RETURNING r.id""",
        (STEP_PAUSED,),
...
def resume_collection_run(conn: psycopg.Connection, run_id: int, pages_total: int) -> dict:
    """A collection run picked up again: its row, showing progress anew."""
    row = conn.execute(
        """UPDATE collection_runs
           SET pages_total = %s, current_step = %s, current_page_ads = 0, heartbeat_at = now()
           WHERE id = %s AND finished_at IS NULL
           RETURNING started_at, total_ads_collected, new_ads_discovered, errors, last_error""",
        (pages_total, STEP_RESUMING, run_id),
    ).fetchone()
    conn.commit()
    if row is None:
        raise RuntimeError(f"run {run_id} is not open any more")
    return row


def pages_by_id(conn: psycopg.Connection, ids) -> list[dict]:
    """Pages as a collection reads them, by their ids."""
    return conn.execute(
...
def companies_seen_in(conn: psycopg.Connection, run_id: int) -> set[int]:
    """The companies whose ads a collection run has stored so far."""
    return {r["company_id"] for r in conn.execute(
        """SELECT DISTINCT a.company_id FROM ad_appearances x JOIN ads a ON a.id = x.ad_id
           WHERE x.collection_run_id = %s""",
        (run_id,),
    ).fetchall()}
...
class StoredAd(NamedTuple):
    ad_id: int
    is_new: bool    # it wasn't in the library before
    counted: bool   # the first time this run stores it (a resumed run may store a Batch again)


def upsert_ad(conn: psycopg.Connection, company_id: int, page_ref_id: int, run_id: int,
              ad: NormalizedAd, now: datetime,
              sort_index: int | None = None) -> StoredAd:
    """Insert/update an ad, and record that this run saw it (once per run). Its
    media is mirrored later, by `mirror_pending_media`, so storing a run's ads takes
    seconds."""
    ...
    appearance = conn.execute(
        """INSERT INTO ad_appearances (ad_id, collection_run_id, is_active, seen_at, sort_index)
           SELECT %(ad)s, %(run)s, %(active)s, %(seen)s, %(sort)s
           WHERE NOT EXISTS (SELECT 1 FROM ad_appearances
                             WHERE ad_id = %(ad)s AND collection_run_id = %(run)s)
           RETURNING id""",
        {"ad": ad_db_id, "run": run_id, "active": ad.is_active, "seen": now, "sort": sort_index},
    ).fetchone()
    conn.commit()
    return StoredAd(ad_db_id, is_new, appearance is not None)
...
class StoreFailed(Exception):
    """Storage refused a file: a problem with the media storage, not with Meta."""
...
def mirror_media(session, url: str, key_without_ext: str, compress_video: bool = True,
    ...
    when the file can't be read from Meta; raises StoreFailed when storage refuses
    it. A video is re-encoded smaller unless `compress_video` is False (Meta's SD
    file is already small); an image is encoded at `image_quality`."""
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            raise StoreFailed(str(exc)) from exc
        return key
...
class MirrorResult(NamedTuple):
    saved: int
    total: int
    not_stored: int = 0             # files storage refused (not Meta's fault)
    store_error: str | None = None  # why, for the last of them


def mirror_pending_media(conn: psycopg.Connection, *, make_session,
                         video_quality: str = "standard", image_quality: str = "standard",
                         workers: int = MIRROR_WORKERS, on_progress=None) -> MirrorResult:
    ...
    written on `conn`, in this thread, as each file is done. A failure leaves the key
    empty, and the page keeps using Meta's URL; the result counts apart the files
    storage refused, which a retry won't fix."""
    jobs = pending_media(conn, video_quality, image_quality)
    if not jobs:
        return MirrorResult(0, 0)
    try:
    ...
    def work(job: MediaJob) -> tuple[MediaJob, str | None, str | None]:
        ...
        try:
            return job, mirror_media(session, job.url, job.key, compress_video=job.compress_video,
                                     image_quality=job.image_quality), None
        except StoreFailed as exc:
            return job, None, str(exc)

    done = mirrored = not_stored = 0
    store_error = None
    try:
        with ThreadPoolExecutor(max_workers=workers) as pool:
            for future in as_completed([pool.submit(work, job) for job in jobs]):
                job, key, refused = future.result()
                done += 1
                if refused is not None:
                    not_stored, store_error = not_stored + 1, refused
                if key:
        ...
        for session in sessions:
            try:
                ...
                pass
    return MirrorResult(mirrored, len(jobs), not_stored, store_error)
