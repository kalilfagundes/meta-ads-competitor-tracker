"""Collection orchestration (async), for either Collection source.
  ...
  - Apify: the Pages go in Batches (up to 5 Pages of one country, as Ad Library
    ...
    and what Apify charged is summed into the run;
...
async def _run_actor(runs, run_input: dict, run: _Run,
                     label: str) -> tuple[dict | None, str, bool]:
    """One Actor run, followed to its end. Returns (the run if it succeeded, else
    None; why it didn't; whether running it again could work). Its cost and the ads
    it has read are kept up to date in the run's progress on every check."""
    try:
        ...
        return None, why, exc.reason != "unauthorized"
    except Exception as exc:
    ...
    if status == "SUCCEEDED":
        ...
        return (None, why, False) if why else (actor_run, "", True)
    return None, f"Actor run {actor_run.get('id')} {status}", True
